...
#define RX_BUFFFER_SIZE            50
	...
	float f_channel[MAX_CHANNEL] = { \
	486.6, \
	487.1,487.3,487.5,487.7,486.3,486.5,486.7};	
#endif
...
uint8_t rx_buffer[RX_BUFFFER_SIZE];    // Length of rx buffer
...
LoRaSV_PROCESS_STATE_t lps = LoRaSV_STATE_TX;
...
static Std_ReturnType LoRaSV_RxHandle(uint8_t* rx_buffer, uint8_t len_rx)
{
	...
	DEBUG_USER("macMsgData.FHDR.DevAddr: %u\r\n", macMsgData.FHDR.DevAddr);
	...
	switch(macMsgData.MHDR.Bits.MType)
	{
		...
		case FRAME_TYPE_COMMAND_CONFIRMED:
		{
			...
			rnd_fchannel = 1; // Random a number for delay time
...
//	char date[14];
	...
	while(1)
	{
		switch(lps)
		{
			...
			{
				DEBUG_USER("\r\n=======TX=======\r\n");
				HAL_GPIO_WritePin(detector_board.stt_pin.gpio_port,detector_board.stt_pin.gpio_pin,1);
...
//				}
			...
			//	content="\0";			
				...
				//state=1; // test
				bat = 100-t;
				sprintf(content,"*000000021,025,20220702025352,1,%u,%u#",bat,state);
				//sprintf(content,"*20220611093300,021,000000021,%u,%u,hello#",state,bat);
				//sprintf(content,"*11,20220808110700#");

			//	DEBUG_USER("Content = %s t=%u bat=%u\r\n",content,t,bat);

			//	memcpy(tx_buffer, "*20201201115600,110,000000002,1,99,hello321#\r\n", detector_board.dataLen);
			  memcpy(tx_buffer, content, detector_board.dataLen);
				t++;
				/* Get Counter from Flash */
				cur_fcnt = flash_read(EEPROM_MSB_ADDR);
				
				DEBUG_USER("Packet = %s is %u -th \r\n",tx_buffer,cur_fcnt);
				  ...
				  ret = LoRaNodeSV_ReceiveData_Normal(&sx127x_node, rx_buffer, &rxlen, SX127X_RX_TIMEOUT_MS);
					if(ret == STD_OK)
					{
						...
						else
						{
							...
							/* Add Code Handle Data Control Message */
							/**/
//							str[0] = rx_buffer[1];
//							str[1] = rx_buffer[2];
//							switch(str[1]){
...
//				}
				...
				{
					DEBUG_USER("\nNo ACK + Timeout => Retransmitting Packet %u...\r\n", cur_fcnt);
					lps = LoRaSV_STATE_TX;
					state_init = 1;
					retry = 1;
					break;
...
//				lps = LoRaSV_STATE_SLEEP;
			...
			case LoRaSV_STATE_SLEEP:
			{
				DEBUG_USER("\r\n=======SLEEP=======\r\n");
				DetectorBuff_Reset();
				HAL_GPIO_WritePin(detector_board.stt_pin.gpio_port,detector_board.stt_pin.gpio_pin,0);
